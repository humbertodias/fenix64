...
void instance_dump(INSTANCE * father, int indent)
{
	...
	proc = i->proc ;
        ...
        if (dcb.NID && dcbproc->ID)
                sprintf (buffer, "%s", getid(dcbproc->ID)) ;
        else
	...
	switch (LOCDWORD(i, STATUS))
	{
		case 0:	strcat (line, "[?] ") ; break ;
		case 1:	strcat (line, "[K] ") ; break ;
		case 2:	strcat (line, "    ") ; break ;
		case 3:	strcat (line, "[S] ") ; break ;
		case 4:	strcat (line, "[F] ") ; break ;
		default:
