...
void gr_mode7_start (int n, int fileid, int inid, int outid,
...
void gr_mode7_draw (int n)
{
	...
	if (LOCDWORD(camera, RESOLUTION))
	{
		camera_x /= LOCDWORD(camera,RESOLUTION) ;
		camera_y /= LOCDWORD(camera,RESOLUTION) ;
	}
	...
	if (outdoor)
	{
		...
		while (~(outdoor_hmask << 1) < outdoor->width-1)  
			outdoor_hmask <<= 1 ;
		while (~(outdoor_vmask << 1) < outdoor->height-1) 
			outdoor_vmask <<= 1 ;
	...
	jump = camera_z < 0 ? -1 : 1 ;
	ptr += (jump > 0 ? dest->pitch : -dest->pitch) ;
	y   += jump ;
	...
	for ( ; y < height && y >= 0 ; y += jump)
	{
		...
		ptr = baseline + (jump > 0 ? dest->pitch : -dest->pitch) ;
	}
	else
	for ( ; y < height && y >= 0 ; y += jump)
	{
		...
		for (x = 0 ; x < width ; x++)
		{
			...
			if (indoor && sx >= 0 && sx < indoor->width &&
				...
				*ptr++ = trans_table[((Uint8*)indoor->data)[indoor->pitch*sy + sx]][*ptr] ;
				...
				if (outdoor) *ptr++ = trans_table[((Uint8*)outdoor->data)[outdoor->pitch*(sy & outdoor_vmask) + (sx & outdoor_hmask)]][*ptr] ;
	...
	while (i)
	{
		if ((LOCDWORD(i,STATUS) == STATUS_RUNNING || LOCDWORD(i,STATUS) == STATUS_FROZEN) 
		     && LOCDWORD(i,CTYPE) == 2)
		{
			...
			if (proclist_count == proclist_reserved)
			{
				...
				proclist = (INSTANCE **) realloc (proclist,
					sizeof(INSTANCE *) * proclist_reserved) ;
			}
	...
	for (nproc = 0 ; nproc < proclist_count ; nproc++)
	{
		...
		if (LOCDWORD (proclist[nproc], RESOLUTION))
		{
			base_x /= LOCDWORD (proclist[nproc], RESOLUTION) ;
			base_y /= LOCDWORD (proclist[nproc], RESOLUTION) ;
			base_z /= LOCDWORD (proclist[nproc], RESOLUTION) ;
		}
		...
		bmp_x = (long) (-((float)FOCAL_DIST * fixtof(x) / fixtof(y)) 
			   * (float)width / (float) dat->focus) ;
		bmp_y = (long) (-((float)FOCAL_DIST * fixtof(z) / fixtof(y)) 
			   * (float)dat->height / (float) dat->focus) ;
