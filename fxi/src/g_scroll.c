...
void gr_scroll_draw (int n, int do_drawing, REGION * clipping)
{
	...
	if (scrolls[n].camera)
	{
		...
		if (LOCDWORD(scrolls[n].camera, RESOLUTION))
		{
			cx /= LOCDWORD(scrolls[n].camera, RESOLUTION) ;
			cy /= LOCDWORD(scrolls[n].camera, RESOLUTION) ;
		}
		cx -= w/2 ;
		cy -= h/2 ;
		...
		if (!(scrolls[n].flags & GRAPH_HWRAP))
			data->x0 = MAX (0, MIN (data->x0, scrolls[n].graph->width - w)) ;
		if (!(scrolls[n].flags & GRAPH_VWRAP))
			data->y0 = MAX (0, MIN (data->y0, scrolls[n].graph->height - h)) ;
	}
	...
	if (scrolls[n].ratio)
	{
		...
		if (!(scrolls[n].flags & BACK_HWRAP))
			data->x1 = MAX (0, MIN (data->x1, scrolls[n].back->width - w)) ;
		if (!(scrolls[n].flags & BACK_VWRAP))
			data->y1 = MAX (0, MIN (data->y1, scrolls[n].back->height - h)) ;
	}
	...
	if (scrolls[n].back)
	{
		if ((scrolls[n].back->flags & F_NCPOINTS) > 0 && 
		...
		while (y < scrolls[n].region->y2)
		{
			...
			{
				gr_blit (0, &r, x+cx, y+cy, 
					 data->flags2, scrolls[n].back) ;
	...
	if ((scrolls[n].graph->flags & F_NCPOINTS) > 0 && 
	...
	while (y < scrolls[n].region->y2)
	{
		...
		while (x < scrolls[n].region->x2)
		{
			gr_blit (0, &r, x+cx, y+cy, data->flags1,
				 scrolls[n].graph) ;
	...
	for (nproc = 0 ; nproc < proclist_count ; nproc++)
	{
		...
		{
			x /= LOCDWORD (proclist[nproc], RESOLUTION) ;
			y /= LOCDWORD (proclist[nproc], RESOLUTION) ;
		}
